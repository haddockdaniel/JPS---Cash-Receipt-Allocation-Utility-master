...
namespace JurisUtilityBase
{
    ...
    {
        public BillDisplay(DataTable AR, JurisUtility ju)
        {
            InitializeComponent();
        ...
        private void buttonPrint_Click(object sender, EventArgs e)
        {
            ...
            if (de.Count == 0)
            {
                ...
                /*
                 * SELECT  dbo.jfn_FormatClientCode(clicode) as clicode, dbo.jfn_FormatMatterCode(MatCode) as matcode, matreportingname,
[ARMBillNbr] as BillNumber, convert(varchar, arbilldate, 101) as BillDate
,ARMFeeBld + ARMCshExpBld + ARMNCshExpBld + ARMSurchgBld + ARMTax1Bld + ARMTax2Bld + ARMTax3Bld + ARMIntBld as BilledAmt
      ,[ARMBalDue] as Balance
      ,[ARMFeeBld] - [ARMFeeRcvd] + [ARMFeeAdj] as Fees
      ,[ARMCshExpBld] - [ARMCshExpRcvd] + [ARMCshExpAdj] as CashExp
      ,[ARMNCshExpBld] - [ARMNCshExpRcvd] + [ARMNCshExpAdj] as NonCashExp
      ,[ARMSurchgBld] - [ARMSurchgRcvd] + [ARMSurchgAdj] as Surcharge
      ,[ARMTax1Bld] - [ARMTax1Rcvd] + [ARMTax1Adj] as Tax1
      ,[ARMTax2Bld] - [ARMTax2Rcvd] + [ARMTax2Adj] as Tax2
      ,[ARMTax3Bld] - [ARMTax3Rcvd] + [ARMTax3Adj] as Tax3
      ,[ARMIntBld] - [ARMIntRcvd] + [ARMIntAdj] as Interest
	  , 0 as Applied
  FROM [ARMatAlloc]
  inner join matter on armmatter = matsysnbr
  inner join client on clisysnbr = matclinbr
  inner join arbill on arbillnbr = armbillnbr
  where armbaldue > 0 and [ARMFeeBld] >= 0 and [ARMCshExpBld] >= 0 and [ARMNCshExpBld] >= 0
                 * 
                 */
                ...
                {
                    sql = "insert into ##CombineChecks ( Client_Code, Matter_Code,  Invoice_Date, Invoice_Number, Bill_Total, Bill_Balance, AR_Allocation, Fees, Cash_Exps, NonCash_Exps, Taxes1, " +
                               " Taxes2, Taxes3, Surcharge, Interest, PPD_Allocation, Trust_Allocation, Other_Allocation, TTYpe, batchNo,  " +
                                ...
                                " values ('" + dr[1].ToString() + "', '" + dr[2].ToString() + "', " +
                                ...
                                " ," + dr[16].ToString() + "," + dr[17].ToString() + "," + dr[18].ToString() + "," + dr[19].ToString() + ", 1, 0, " +
